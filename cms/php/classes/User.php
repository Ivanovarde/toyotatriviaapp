...
class User extends ABM {
	...
	private static function getUser($sql){
		...
		Log::l('User getUser', $record, true);
		return $record;
	}
	...
	private static function getUsers($sql) {
		...
		//	 $vAll[] = new User($record["member_id"]);
		//}
		//return $vAll;
		...
		$vAll = array();
		foreach($rsAll as $record) {
			 array_push($vAll, new User($record->member_id));
		}
		return $vAll;
	}
	...
	public static function by_username($username){
		$sql = "SELECT " . Tables::USERS . ".member_id FROM " . Tables::USERS . " WHERE username = '" . $username . "' ";
		Log::l('User by_username', $sql, true);
		return self::getUser($sql);
	}
	...
	public function generatePassword(){
		...
		$maxlen = 32;
		$pw = new String();
		$password = $pw->randomText(rand($minlen,$maxlen));
		return $password;
	}
