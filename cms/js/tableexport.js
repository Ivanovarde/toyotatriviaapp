	...
	$.fn.tableExport = function (options, isUpdate) {
		...
		$this.each(function () {
			...
			function checkCaption(exportButton) {
				...
				!$caption.length ? $el.prepend('<caption class="' + bootstrapSpacing + settings.position + '">' + exportButton + '</caption>') : ($caption.find('button').length < 3 ? $caption.append(exportButton) : '') ;
				console.log($caption.find('button').length);
			}
